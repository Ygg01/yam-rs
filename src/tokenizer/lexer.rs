...
use super::reader::{
    is_flow_indicator, is_newline, is_plain_unsafe, is_valid_escape, is_valid_skip_char,
    is_white_tab,
};
use crate::tokenizer::ErrorType;
...
macro_rules! impl_quote {
    ($quote:ident($quote_start:expr), $trim:ident($trim_fn:ident, $lit:literal), $start:ident($quote_fn:ident) => $match_fn:ident) => {
        fn $quote<R: Reader<B>>(&mut self, reader: &mut R) -> NodeSpans {
...
impl<B> Lexer<B> {
    ...
    fn try_parse_anchor<R: Reader<B>>(&mut self, reader: &mut R) -> Option<(usize, usize)> {
        ...
        let is_anchor_inline = self.skip_separation_spaces(reader).0 == 0;
        if is_anchor_inline {
            Some(anchor)
        } else {
            self.tokens.push_back(ANCHOR);
    ...
    fn fetch_flow_node<R: Reader<B>>(&mut self, reader: &mut R) {
        ...
        if matches!(self.curr_state(), DocBlock) {
            self.set_state(LexerState::AfterDocBlock);
        }
    }
    ...
    fn get_scalar_node<R: Reader<B>>(&mut self, reader: &mut R, chr: u8) -> NodeSpans {
        ...
        {
            reader.consume_bytes(1);
            self.push_error_token(ErrorType::InvalidScalarStart, &mut scal_spans);
            NodeSpans {
                col_start: 0,
                is_multiline: false,
                spans: scal_spans,
            ...
            let scal = self.process_single_quote(reader);
            ...
            let mut spans = self.prepend_tags_n_anchor();
            let scal = self.process_double_quote(reader);
            spans.extend(scal.spans);
            NodeSpans {
                col_start: scal.col_start,
                is_multiline: scal.is_multiline,
        ...
        loop {
            ...
            if is_white_tab_or_break(chr) {
                ...
                if matches!(seq_state, BeforeElem | BeforeFirstElem) {
                    self.push_error_token(
                        ErrorType::ExpectedNodeButFound { found: ',' },
                        &mut spans,
                    );
                }
                ...
                )
            {
                match peek_next {
                    b'[' | b'{' | b'}' => {
                        self.push_error_token(
                            ErrorType::UnexpectedSymbol(peek_next as char),
                            &mut spans,
                        );
    ...
    fn get_flow_map<R: Reader<B>>(&mut self, reader: &mut R, init_state: MapState) -> NodeSpans {
        ...
        let col_start = reader.col();
        let is_nested = init_state == MapState::AfterColon;
    ...
    fn prepend_tags_n_anchor(&mut self) -> Vec<usize> {
        ...
        if let Some(anchor) = take(&mut self.prev_anchor) {
            if self.had_anchor {
                self.push_error_token(ErrorType::NodeWithTwoAnchors, &mut tokens);
            }
            tokens.push(ANCHOR);

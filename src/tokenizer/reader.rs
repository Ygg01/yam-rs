...
pub trait Reader<B> {
    ...
    fn skip_space_tab(&mut self, buf: &mut B) -> usize;
    fn consume_bytes(&mut self, buf: &mut B, amount: usize) -> usize;
    ...
    fn consume_anchor_alias(&mut self, buf: &mut B) -> (usize, usize);
    fn read_tag(&mut self, buf: &mut B) -> (Option<ErrorType>, usize, usize, usize);
    fn read_tag_handle(&mut self, buf: &mut B) -> Result<Vec<u8>, ErrorType>;
    fn read_tag_uri(&mut self, buf: &mut B) -> Option<(usize, usize)>;
    fn read_break(&mut self, buf: &mut B) -> Option<(usize, usize)>;
